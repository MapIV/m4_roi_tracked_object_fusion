...
: Node(node_name, options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
{
  ...
  // initialize debug tool
  {
    ...
    debug_publisher_ = std::make_unique<DebugPublisher>(this, "roi_tracked_object_fusion");
...
template class FusionNode<TrackedObjects, TrackedObject>;
template class FusionNode<DetectedObjectsWithFeature, DetectedObjectWithFeature>;
template class FusionNode<sensor_msgs::msg::PointCloud2, TrackedObjects>;
