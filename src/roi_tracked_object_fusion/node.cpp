...
#include "roi_tracked_object_fusion/roi_tracked_object_fusion/node.hpp"
...
RoiTrackedObjectFusionNode::RoiTrackedObjectFusionNode(const rclcpp::NodeOptions & options)
: FusionNode<TrackedObjects, TrackedObject>("roi_tracked_object_fusion", options)
{
...
void RoiTrackedObjectFusionNode::preprocess(TrackedObjects & output_msg)
{
...
void RoiTrackedObjectFusionNode::fuseOnSingleImage(
  const TrackedObjects & input_object_msg, const std::size_t image_id,
  const DetectedObjectsWithFeature & input_roi_msg,
  const sensor_msgs::msg::CameraInfo & camera_info,
  TrackedObjects & output_object_msg __attribute__((unused)))
{
...
RoiTrackedObjectFusionNode::generateDetectedObjectRoIs(
  ...
  const Eigen::Affine3d & object2camera_affine, const Eigen::Matrix4d & camera_projection)
{
  ...
  for (std::size_t obj_i = 0; obj_i < input_object_msg.objects.size(); ++obj_i) {
    ...
    object_roi.object = object_recognition_utils::toDetectedObject(object);
...
void RoiTrackedObjectFusionNode::fuseObjectsOnImage(
...
bool RoiTrackedObjectFusionNode::out_of_scope(const TrackedObject & obj)
{
  bool is_out = true;
...
void RoiTrackedObjectFusionNode::publish(const TrackedObjects & output_msg)
{
  ...
  TrackedObjects output_objects_msg, debug_fused_objects_msg, debug_ignored_objects_msg;
  ...
  debug_publisher_->publish<TrackedObjects>("debug/fused_objects", debug_fused_objects_msg);
  debug_publisher_->publish<TrackedObjects>("debug/ignored_objects", debug_ignored_objects_msg);
...
RCLCPP_COMPONENTS_REGISTER_NODE(roi_tracked_object_fusion::RoiTrackedObjectFusionNode)
